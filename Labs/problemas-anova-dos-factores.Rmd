...
```{r 3_load, eval=solucion, echo=solucion}
...
```{r 1_load}
peso_rata %>% dplyr::rename(Alta.proteína = Alta.proteína.1,
                            Baja.proteína = Baja.proteína.1) %>% 
...
peso_rata_smr <- summary(rata_supuestos$estimated_means)
...
```{r 6_plot_res, fig.cap="Figura 6.3 -- Gráfico diagnostico de ANOVA humedad ~ tipo * laboratorio", eval=solucion, echo=solucion}
