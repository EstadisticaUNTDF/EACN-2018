...
```{r interaccion1, echo = FALSE, fig.height=4, fig.cap="(ref:interaccion1)", fig.show='hold'}
aprendizaje_1 <- tribble(
 ~Sexo , ~Adolescente , ~Adulto_joven , ~Anciano ,
 "Masculino" , 9 , 11 , 16 ,
 "Femenino" , 9 , 11 , 16 
) %>% 
  gather(Edad, Tiempo, -Sexo)

aprendizaje_2 <- tribble(
 ~Sexo , ~Adolescente , ~Adulto_joven , ~Anciano ,
 "Masculino" , 11 , 13 , 18 ,
 "Femenino" , 7 , 9 , 14 
) %>% 
...
list(a = aprendizaje_1, b = aprendizaje_2, c =aprendizaje_3) %>% 
  bind_rows(.id = "Tabla") %>% 
  ggplot(aes(Sexo, Tiempo, color = Edad)) +
  geom_point() +
  geom_line(aes(group = Edad)) +
  facet_wrap(~Tabla) +
  theme(strip.background = element_blank(),
        strip.text.x = element_text(hjust = 0))

list(d = aprendizaje_1, e = aprendizaje_2, f =aprendizaje_3) %>% 
  bind_rows(.id = "Tabla") %>% 
  ggplot(aes(Edad, Tiempo, color = Sexo)) +
  geom_point() +
...
```{r interaccion-no-importante, echo=FALSE, fig.align='default', out.width="50%", fig.height = 4, fig.cap = "(ref:interaccion-no-importante)", fig.show= 'hold'}
aprendizaje_4 <- tribble(
 ~Sexo , ~Adolescente , ~Adulto_joven , ~Anciano ,
 "Masculino" , 9.75 , 12 , 17.25 ,
 "Femenino" , 8.25 , 10 , 14.75 
) %>% 
  gather(Edad, Tiempo, -Sexo)
...
```{r datos-asma, echo=FALSE}
asma <- expand.grid(Farmaco = rep(LETTERS[1:3], each = 4),
                    Estación = c("Primavera", "Verano", "Otoño", "Invierno"), stringsAsFactors = FALSE) %>%
  as_tibble() %>% 
  mutate(Virulencia = c(23, 28, 32, 18,	56, 58, 53, 55,	42, 41, 36, 37,
                        32, 41, 43, 48,	64, 58, 67, 72,	51, 53, 55, 60,
                        18, 16, 21, 10,	48, 50, 47, 47,	28, 31, 23, 33,
                        30, 40, 33, 47,	60, 61, 63, 59,	56, 60, 61, 55))
...
```{r graficos-asma, echo=FALSE, fig.show='hold', fig.cap="(ref:grafico-asma)"}
p1 <- asma %>% group_by(Estación, Farmaco) %>% 
  summarise(media = mean(Virulencia)) %>% 
  ggplot(aes(Estación, media, colour = Farmaco, shape = Farmaco, 
             linetype = Farmaco, group = Farmaco)) +
  geom_point(size = 2) +
  geom_line(size = 1) +
  labs(y = "Virulencia")
p2 <- ggplot(asma, aes(Estación, Virulencia)) +
  geom_boxplot()
p3 <- ggplot(asma, aes(Farmaco, Virulencia)) +
  geom_boxplot()
top_row <- plot_grid(p2, p3, labels = "auto")

plot_grid(top_row, p1, labels = c("", "c"), ncol = 1)
```
...
asma <- mutate(asma, 
               Farmaco = as.factor(Farmaco),
               Estación = as.factor(Estación))
contrasts(asma$Farmaco) <- contr.sum
contrasts(asma$Estación) <- contr.sum
asma_aov <- aov(Virulencia ~ Estación * Farmaco, data = asma)
Anova(asma_aov, type = "III") %>% kable(digits = ,caption = "(#tab:anova-asma)")
```
...
summary(asma_aov, split = list(Farmaco = list("A vs B"= 1, "B vs C" = 2),
                               Estación = list("Invierno vs Verano" = 1,
                                               "Otoño vs Verano" = 2, 
                                               "Primavera vs Verano" = 3))) %>% 
  kable()
...
```{r, echo=FALSE, eval=FALSE}
summary(asma_aov, split = list(Estación = list("Invierno vs Verano" = 1)), expand.split = FALSE) %>% 
  kable()()
```
...
BvAC <- c(-1, 2, -1)
mat <- cbind(BvAC)
contrasts(asma$Farmaco) <- mat
asma_aov_farmacos <- aov(Virulencia ~ Estación * Farmaco, data = asma)
summary(asma_aov_farmacos, split = list(Farmaco = list("B vs A-C" = 1)), expand.split = FALSE) %>% 
  kable()
```
...
```{r, echo=FALSE, eval=FALSE}
asma <- mutate(asma,
               FxE = interaction(Farmaco, Estación))

cvc <- c(0, 0, -1, 0, 0, 3, 0, 0, -1, 0, 0, -1)
cvc <- cbind(cvc)
contrasts(asma$FxE) <- cvc
asma_aov_c <- aov(Virulencia ~ FxE, data = asma)
summary(asma_aov_c, split = list(FxE = list("C Otoño vs C Otros" = 1)), expand.split = FALSE) %>% 
  kable()
...
```{r, echo=FALSE, eval=FALSE}
asma_lsd <- lsmeans(asma_aov, pairwise ~ Estación:Farmaco, adjust = "none") %>% summary %>% .$contrasts
asma_tukey <- lsmeans(asma_aov, pairwise ~ Estación:Farmaco, adjust = "tukey")%>% summary %>% .$contrasts

cbind(asma_lsd, p.value.tukey = asma_tukey$p.value) %>% 
  rename(p.value.lsd = p.value) %>% 
  kable()(split.tables = Inf)
```
...
```{r, echo=FALSE, eval=FALSE}
 means <- lsmeans(asma_aov, pairwise ~ Estación:Farmaco, adjust = "none") %>% summary %>% .$lsmeans
 asma_pairwise_matrix <- matrix(NA, nrow = length(levels(asma$FxE)), ncol = length(levels(asma$FxE)))
 rownames(asma_pairwise_matrix) <- colnames(asma_pairwise_matrix) <- 
   with(asma, levels(interaction(Estación, Farmaco)))
 asma_pairwise_matrix[lower.tri(asma_pairwise_matrix)] <- asma_lsd$p.value
 asma_pairwise_matrix <- t(asma_pairwise_matrix)
 lt <- lower.tri(asma_pairwise_matrix)
 asma_pairwise_matrix[lt] <- asma_tukey$p.value[order(row(lt)[lt], col(lt)[lt])]
diag(asma_pairwise_matrix) <- means$lsmean 
which_means <- which(diag(1, 12, 12)==1, arr.ind = TRUE)
asma_pairwise_matrix %>% kable()(split.tables = Inf, 
                                emphasize.strong.cells = which(asma_pairwise_matrix<0.05, arr.ind = TRUE),
                                emphasize.italics.cells = which(asma_pairwise_matrix>1, arr.ind = TRUE))
```
...
```{r citricos, echo=FALSE, fig.show='hold', fig.align='default', out.width="50%", fig.cap="(ref:citricos)"}
citricos <- expand.grid(Especie = c("Naranja", "Toronja", "Mandarina"),
                        Sombra = c("Sol", "Media Sombra", "Sombra")) %>% 
  mutate(LAM = c(112, 90, 123, 86, 73, 89, 80, 62, 81))

ggplot(citricos, aes(Especie, LAM, colour = Sombra, shape = Sombra, linetype = Sombra, group = Sombra)) +
  geom_point(size = 2) +
  geom_line(size = 1)

ggplot(citricos, aes(Sombra, LAM, colour = Especie, shape = Especie, linetype = Especie, group = Especie)) +
  geom_point(size = 2) +
  geom_line(size = 1)
...
```{r, echo=FALSE, eval=FALSE}
tukey.test(matrix(citricos$LAM, nrow = 3, byrow = TRUE)) %>% kable()
```


```{r, echo=FALSE, eval=FALSE}
contrasts(citricos$Especie) <- contr.sum
contrasts(citricos$Sombra) <- contr.sum

citricos_aov <- aov(LAM ~ Especie+Sombra, data = citricos)
summary(citricos_aov) %>% kable()
```
