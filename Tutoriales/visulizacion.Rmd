...
Para saber que es cada una de las columnas podemos ver la ayuda con `?mpg`:
...
1. Prueben correr `ggplot(data = mpg)`. ¿Qué ven?  
```{r solo-ggplot, exercise=TRUE}
...
question("¿Cuantas columnas y filas hay en `mpg`?",
  answer("8 columnas y 10 filas"),
  answer("11 columnas y 10 filas"),
  answer("11 columnas y 234 filas", correct = TRUE),
  answer("11 columnas y 224 filas")
)
```

3. Hagan un gráfico de dispersión entre `cty` vs `hwy`  

```{r dispersion-hwy-cyl, exercise=TRUE}

```

4. Hagan un gráfico de dispersión entre `class` y `fl`. ¿Qué ven? ¿Porqué el
gráfico no es útil?

```{r dispersion-class-drv, exercise=TRUE}
...
1.  ¿Qué está mal con el siguiente código? ¿Porqué no son azules los puntos?
    Corrijanlo

    ```{r puntos-azules, exercise = TRUE, exercise.eval = TRUE}
    ggplot(data = mpg) + 
      geom_point(mapping = aes(x = displ, y = hwy, color = "blue"))
    ```
    
1.  ¿Cuáles son las variables categóricas en `mpg`? ¿Cuáles son las variables
    continuas?
    
    ¿Cómo puedes ver esta información cuando corres `mpg`?
    
    ```{r ayuda-mpg, exercise=TRUE}
    
    ```
    
    <div id="ayuda-mpg-hint">
    **Pista**: escribe `?mpg` para leer la documentación del conjunto de datos.
    </div>

1.  Mapeen una variable continua a `color`, `size`y `shape`.
    ¿Cuál es la diferencia en su acción entre variables categóricos y continuas?
    
    ```{r mapeo-continua, exercise = TRUE}
    
    ```
    
    
1.  ¿Qué pasa si mapean la misma variable a varias estéticas?

     ```{r mapeo-varias, exercise = TRUE}
    
    ```
    
    <div id="mapeo-varias-hint">
    **Pista**: Por ejemplo class a color y forma de punto.
    </div>

1.  ¿Qué hace la estética `stroke`? ¿Con qué formas funciona?

    
     ```{r ayuda-stroke, exercise=TRUE}
    
    ```
    
    <div id="ayuda-stroke-hint">
    **Pista**:  usen `?geom_point`.
    </div>
    
1.  ¿Qué sucede cuando mapean una estética a algo que no sea un nombre de 
    variable como `aes(colour = displ < 5)`?  
    

    ```{r mapeo-no-variable, exercise=TRUE, exercise.eval = FALSE }
        ggplot(data = mpg) +
...
Comprueben que ambos gráficos son iguales.
...
1.  ¿Cuál geom usarias para dibujar un gráfico de líneas? ¿Un boxplot? ¿Un histograma?
    ¿Un gráfico de áreas?
    

1.  Corre este código mentalmente y predice cómo se verá el gráfico.
    Luego, ejecuta el código y comprueba lo que pensaste.
    
    ```{r global-mapping, exercise=TRUE}
    ggplot(data = mpg, mapping = aes(x = displ, y = hwy, color = drv)) + 
      geom_point() + 
      geom_smooth(se = FALSE)
    ```

1.  ¿Qué crees que hace `show.legend = FALSE`? ¿Qué sucede si lo quitas?
    ¿Por qué crees que lo usé antes en el capítulo?
    
    ```{r show-legend, exercise=TRUE}
ggplot(data = mpg) +
  geom_smooth(
    mapping = aes(x = displ, y = hwy, color = drv),
    show.legend = FALSE
...
1.  ¿Qué hace el argumento `se` de `geom_smooth()`?

    ```{r se-smooth, exercise=TRUE}
    ggplot(data = mpg, mapping = aes(x = displ, y = hwy, color = drv)) + 
      geom_point() + 
      geom_smooth(se = TRUE)
    ```
1.  ¿Serán diferentes estos dos gráficos? ¿Por qué sí o no?

    ```{r graficos-iguales, exercise = TRUE}
    ggplot(data = mpg, mapping = aes(x = displ, y = hwy)) + 
      geom_point() + 
      geom_smooth()
...
1.  Recrea el código para generar los siguientes gráficos:
...
```{r recreando-graficos, exercise = TRUE}
...
1. ¿Cuál es el *geom* asociado por defecto a `stat_summary`? ¿Cómo podrías 
reescribir el gráfico anterior para usar la función *geom* en vez la función *stat*?

```{r stat-summary-ex, exercise = TRUE}
ggplot(data = diamonds) +
  stat_summary(
    mapping = aes(x = cut, y = depth),
    fun.y = median,
...
```
2. ¿Qué hace `geom_col`? ¿En que se diferencia de `geom_bar`?
...
3. La mayoría de los *geoms* y *stats* vienen en pares que casi siempre son 
usados juntos. Lee la documentación y haz una lista de todos los pares. 
¿Qué tienen en común? 

4. ¿Qué variables computa `stat_smooth()` ¿Qué parámetros controlan su 
comportamiento?

5. En el gráfico de proprociones, necesitamos poner `group = 1`. ¿Por qué? En 
...
1. ¿Cómo podrías mejorar el siguiente gráfico?

```{r overplot, exercise = TRUE, exercise.eval = TRUE}
ggplot(data = mpg) +
  geom_point(mapping = aes(x = hwy, y = cty))
```

2. ¿Cómo se controla el grado de separación en `position_dodge`?

3. ¿Cómo se podría controlar el grado de ruido en `position_jitter`?
...
1. Inviertan los ejes de un gráfico de barras.

```{r barras-invertidas, exercise = TRUE}
ggplot(data = diamonds) + 
   geom_bar(mapping = aes(x = cut, fill = clarity), position = "dodge")
```
...
3.¿Para qué sirve la función `labs()`?
...
1. ¿Qué sucede si modificas algún elemento particular del gráfico con `theme()`
...
2. Pon la leyenda arriba.

```{r leyenda-arriba, exercise = TRUE}
ggplot(data = mpg) +
  geom_bar(mapping = aes(x = class, fill = fl)) +
  theme(legend.position = "")
```

3. Cambia el tipo de forma a puntos abiertos

```{r puntos-abiertos, exercise = TRUE, exercise.eval=FALSE}
ggplot(data = mpg) +
  geom_point(mapping = aes(x = displ, y = hwy, colour = class)) +
  scale_ #completar
```
